...
namespace Mapbox.BaseModule.Data
{
	...
	public struct LatitudeLongitudeBounds
	{
		...
		/// <summary> Initializes a new instance of the <see cref="LatitudeLongitudeBounds" /> struct. </summary>
		...
		public LatitudeLongitudeBounds(LatitudeLongitude sw, LatitudeLongitude ne)
		{
			this.SouthWest = sw;
			this.NorthEast = ne;
		}
		...
		public static LatitudeLongitudeBounds FromCoordinates(LatitudeLongitude a, LatitudeLongitude b)
		{
			var bounds = new LatitudeLongitudeBounds(a, a);
			bounds.Extend(b);
		...
		public static LatitudeLongitudeBounds World()
		{
			var sw = new LatitudeLongitude(-90, -180);
			var ne = new LatitudeLongitude(90, 180);

			return new LatitudeLongitudeBounds(sw, ne);
		}

		/// <summary> Extend the bounding box to contain the point. </summary>
		/// <param name="point"> A geographic coordinate. </param>
		public void Extend(LatitudeLongitude point)
		{
		...
		public void Extend(LatitudeLongitudeBounds bounds)
		{
		...
		// {
		// 	var test = (tileBounds.TopLeft.Latitude <= this.East && tileBounds.BottomRight.Latitude >= this.West) &&
		// 	           (tileBounds.BottomRight.Longitude <= this.North && tileBounds.TopLeft.Longitude >= this.South);
		// 	return test;
		// }
	}
}
