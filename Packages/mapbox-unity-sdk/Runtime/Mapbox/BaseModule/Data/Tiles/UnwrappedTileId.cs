...
namespace Mapbox.BaseModule.Data.Tiles
{
	...
	public struct UnwrappedTileId : IEquatable<UnwrappedTileId>
	{
		...
		public UnwrappedTileId(CanonicalTileId canonicalTileId)
		{
			this.Z = canonicalTileId.Z;
			this.X = canonicalTileId.X;
			this.Y = canonicalTileId.Y;
		}
		...
		///     <see cref="T:Mapbox.BaseModule.Data.Tiles.UnwrappedTileId"/>.
		/// </summary>
		/// <returns>
		...
		{
			yield return new UnwrappedTileId(this.Z + 1, this.X * 2, this.Y * 2);
			yield return new UnwrappedTileId(this.Z + 1, this.X * 2 + 1, this.Y * 2);
			yield return new UnwrappedTileId(this.Z + 1, this.X * 2 + 1, this.Y * 2 + 1);
			yield return new UnwrappedTileId(this.Z + 1, this.X * 2, this.Y * 2 + 1);
		}
	}
}
